...
export default function TransactionsPage({ searchTerm = '' }: { searchTerm?: string }) {
  const [transactions, setTransactions] = useState<Transaction[]>([]);

  useEffect(() => {
    // Müşteriler sayfasında eklenen verileri al
    const savedCustomers = localStorage.getItem('app_customers');
    if (savedCustomers) {
      const customers = JSON.parse(savedCustomers);
      
      // Müşteri verilerini işlem listesine dönüştür
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">
          İşlemler / Transacties
        </h1>
        <p className="text-xs text-slate-500 mt-1">
          Tüm müşteri ödeme hareketleri / Alle betalingen van klanten
        </p>
      </div>
              ...
              {filteredTransactions.length > 0 ? (
                ...
                <tr>
                  <td colSpan={4} className="p-6 text-center text-slate-400">
                    İşlem kaydı bulunamadı. / Geen transacties gevonden.
                  </td>
                </tr>
              )}
            </tbody>
